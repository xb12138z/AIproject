...
public:
    ...
    sql::ResultSet* executeQuery(const std::string& sql, Args&&... args)//查数据
    {
        ...
        try 
        {
            ...
            bindParams(stmt.get(), 1, std::forward<Args>(args)...);// 执行查询并返回结果
            //executeQuery("SELECT id FROM users WHERE username = ? AND password = ?", username, password);参数会自动绑定
            
            return stmt->executeQuery();// 注意：调用者需要负责删除返回的 ResultSet 对象
        } 
    ...
    template<typename... Args>
    int executeUpdate(const std::string& sql, Args&&... args)//执行修改
    {
        std::lock_guard<std::mutex> lock(mutex_);
        try 
        {
