...
{
public:
    using HttpCallback = std::function<void (const http::HttpRequest&, http::HttpResponse*)>;// HTTP 请求处理回调函数类型
    
    // 构造函数
    HttpServer(int port,
               const std::string& name,
               bool useSSL = false,
               muduo::net::TcpServer::Option option = muduo::net::TcpServer::kNoReusePort);// 构造函数，初始化服务器参数
    ...
    muduo::net::EventLoop* getLoop() const // 获取主线程的事件循环指针
    { 
        return server_.getLoop(); 
    }

    void setHttpCallback(const HttpCallback& cb)// 设置HTTP请求处理回调函数
    {
        httpCallback_ = cb;
    }
