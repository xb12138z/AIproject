    ...
    , database_(database)
{
    try 
    {
        ...
        if (conn_) 
        {
            ...
            conn_->setClientOption("OPT_CONNECT_TIMEOUT", "10");// 设置连接超时时间
            conn_->setClientOption("multi_statements", "false");// 禁止多语句执行（防sql注入）
...
bool DbConnection::ping() 
{
    try 
    {
        ...
        std::unique_ptr<sql::Statement> stmt(conn_->createStatement());// 创建新的语句对象
        std::unique_ptr<sql::ResultSet> rs(stmt->executeQuery("SELECT 1"));// 简单查询测试连接
        return true;
    } 
    catch (const sql::SQLException& e) 
    {
