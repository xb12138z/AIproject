...
void CorsMiddleware::before(HttpRequest& request) // 处理CORS预检请求（OPTIONS方法）
{
...
void CorsMiddleware::after(HttpResponse& response) // 添加CORS相关的响应头
{
...
bool CorsMiddleware::isOriginAllowed(const std::string& origin) const// 检查请求的Origin是否在允许的列表中
{
...
void CorsMiddleware::handlePreflightRequest(const HttpRequest& request, 
                                          HttpResponse& response) // 处理CORS预检请求，设置适当的响应头
{
...
void CorsMiddleware::addCorsHeaders(HttpResponse& response, 
                                  const std::string& origin) // 添加CORS相关的响应头
{
    try 
    {
...
// 工具函数：将字符串数组连接成单个字符串
std::string CorsMiddleware::join(const std::vector<std::string>& strings, const std::string& delimiter) // 将字符串数组连接成单个字符串，使用指定的分隔符
{
